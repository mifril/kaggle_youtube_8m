...
def train():
    if not os.path.exists('weights5'): os.mkdir('weights5')
    ...
    model = build_mod()
    if True:
      wfn = sorted(glob.glob('weights5/*.h5'))[-1]
    ...
    for e in range(n_eph):
        ...
        for d in npz_itr('train'):
            ...
            if cnt % n_itr == 0:
                ...
                model.save_weights('weights5/%0.5f_%d_%d.h5' % (g, e, cnt))
