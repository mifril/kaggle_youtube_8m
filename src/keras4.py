...
def npz_itr(tp='test', batch=1024):
...
# def npz_itr(path='test', batch=1024):
#     files = sorted(glob.glob(os.path.join(FOLDER_NPZ, path, '*npz')))
#     print('total files in %s %d' % (path, len(files)))
#     ids, aud, rgb, lbs = [], [], [], []
#     for fn in files:
#         loaded = np.load(fn)
#         ids.append(loaded['ids'])
#         rgb.append(loaded['rgb'])
#         aud.append(loaded['audio'])
#         lbs.append(loaded['labels'])
#         length = np.concatenate(np.array(ids)).shape[0]
#         # print (length, batch)
#         if length >= batch:
#           # print ('here')
#           if path != 'test':
#             lbs = np.concatenate(lbs)
#             labels_ohe = np.zeros((length, 4716)).astype(np.uint8)
#             for i in range(length):
#                 labels_ohe[i, lbs[i]] = 1

#             yield np.concatenate(ids), np.concatenate(aud),\
#                 np.concatenate(rgb), labels_ohe
#             # print ('here2')
#             ids, aud, rgb, lbs = [], [], [], []
#             # print ('here3')
...
def train():
    if not os.path.exists('weights4'): os.mkdir('weights4')
    ...
    # _, x1_val, x2_val, y_val = next(npz_itr('val', 20000))

    model = build_mod()
    if True:
      wfn = sorted(glob.glob('weights4/*.h5'))[-1]
    ...
    for e in range(n_eph):
        ...
        # for d in tf_itr('train', batch):
            ...
            if cnt % n_itr == 0:
                ...
                model.save_weights('weights4/%0.5f_%d_%d.h5' % (g, e, cnt))
...
def predict(out_file_location):
    with open(out_file_location, "w+") as out_file:
      ...
      model.load_weights(wfn)
      print('loaded weight file: %s' % wfn)
      # idx, x1_val, x2_val, _ = next(npz_itr('test', 700640))
      # ypd = []
      # idx = []
      cnt = 0

      out_file.write("VideoId,LabelConfidencePairs\n")
      for d in npz_itr('test', 1024):
          print ('batch ', cnt)
      ...
      # print(np.array([val for sublist in ypd for val in sublist]).shape)
      # with Pool() as pool:
      #     out = pool.map(conv_pred, [val for sublist in ypd for val in sublist])
      # print (np.array(ypd).shape)
      # out = conv_pred([val for sublist in ypd for val in sublist])

      # idx = [val for sublist in idx for val in sublist]
      # df = pd.DataFrame.from_dict({'VideoId': idx, 'LabelConfidencePairs': out})
      # df.to_csv(, header=True, index=False, columns=['VideoId', 'LabelConfidencePairs'])


if __name__ == '__main__':
    # train()
    predict('../output/keras_nn_4.csv')
